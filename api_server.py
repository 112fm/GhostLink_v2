...
TARIFF_FLOOR = {
    "regular": {1: 100, 2: 100, 3: 150, 4: 200, 5: 250},
    "own": {1: 75, 2: 75, 3: 125, 4: 150, 5: 175},
}
...
def calc_price(devices: int, tier: str):
    ...
    if tier == "vip":
        ...
        tier = "own"
    price = TARIFF_BASE[devices]
    min_pay = TARIFF_FLOOR[tier][devices]
...
def api_tariffs(
    ...
    prices = {}
    for d in range(1, 6):
        price, min_pay, _ = calc_price(d, tier)
        prices[str(d)] = {"price": price, "min_pay": min_pay}
...
def subscribe(
    ...
    rec["device_limit"] = 2 if tariff_id == "solo" else devices
    tier = _norm_tier(rec.get("member_tier"))
...
def admin_stats(
    ...
    traffic_up = 0
    traffic_down = 0
    inbounds = panel_get_inbounds() or []
    for ib in inbounds:
        traffic_up += int(ib.get("up", 0))
        traffic_down += int(ib.get("down", 0))
