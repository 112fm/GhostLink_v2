...
def get_admins(db=None):
    if db is None:
        from bot.storage import load_db
        db = load_db()
    admins = [str(ADMIN_ID)]
    meta = db.get("_meta", {})
...
def is_admin(user_id, db=None) -> bool:
    return str(user_id) in get_admins(db)
...
async def notify_admins(bot, text, reply_markup=None):
    ...
    for a in get_admins(db):
        try:
            await bot.send_message(a, text, reply_markup=reply_markup)
        except Exception:
            pass
...
def user_kb(status: str, is_admin: bool = False, user_id: str = None):
    kb = InlineKeyboardMarkup(row_width=1)
    pwa_url = pwa_base_url()
    if pwa_url and user_id:
        token = issue_pwa_login_token(str(user_id))
        pwa_url = build_url_with_query(pwa_url, login_token=token)
    if is_admin:
        if pwa_url:
            kb.add(InlineKeyboardButton("🌟 Открыть приложение", url=pwa_url))
        kb.add(
        ...
        return kb
    if status in ("approved", "trial", "active", "vip") and pwa_url:
        kb.add(InlineKeyboardButton("🌟 Открыть приложение", url=pwa_url))
    kb.add(
        InlineKeyboardButton("🆘 Поддержка", callback_data="support"),
        InlineKeyboardButton("📜 Устав клуба", callback_data="club_rules"),
...
def pwa_base_url() -> str:
    if (PWA_URL or "").strip():
        return (PWA_URL or "").strip()
    url = (WEBAPP_URL or "").strip()
    if not url:
        return ""
    out = url.replace("webapp-mini", "webapp-pwa").replace("веб-приложение-mini", "веб-приложение-pwa")
    parts = urlsplit(out)
    return urlunsplit((parts.scheme, parts.netloc, parts.path, "", ""))


def build_url_with_query(base_url: str, **params) -> str:
    if not base_url:
        return ""
...
def issue_pwa_login_token(tg_id: str, ttl_sec: int = 300) -> str:
    db = load_db()
    meta = db.get("_meta", {})
    now = int(datetime.datetime.now().timestamp())
    tokens = meta.get("pwa_login_tokens") or {}
    clean = {}
    for t, info in tokens.items():
        try:
            exp = int((info or {}).get("exp") or 0)
            if exp > now:
                clean[str(t)] = info
        except Exception:
            continue
    token = uuid.uuid4().hex + uuid.uuid4().hex
    clean[token] = {"tg_id": str(tg_id), "exp": now + int(ttl_sec)}
    meta["pwa_login_tokens"] = clean
    db["_meta"] = meta
    save_db(db)
    return token


def issue_pwa_code(tg_id: str, ttl_sec: int = 900) -> str:
    db = load_db()
    meta = db.get("_meta", {})
    now = int(datetime.datetime.now().timestamp())
    codes = meta.get("pwa_codes") or {}
    clean = {}
    ...
    clean[code] = {"tg_id": str(tg_id), "exp": now + int(ttl_sec)}
    meta["pwa_codes"] = clean
    db["_meta"] = meta
    save_db(db)
    return code


def ensure_user_trial(db: dict, tg_id: str, email_hint: str = ""):
    u = db.get(str(tg_id)) or {}
    u = ensure_traffic_defaults(u)
    u["status"] = "trial"
    u["expiry"] = (datetime.date.today() + datetime.timedelta(days=TRIAL_DAYS)).strftime("%Y-%m-%d")
    u["device_limit"] = int(u.get("device_limit") or 1)
    u["tariff_name"] = u.get("tariff_name") or "trial_7d"
    u["traffic_warn80"] = 0
    u["traffic_warn95"] = 0
    ...
    if not u.get("uuid"):
        ...
        email = email_hint or f"tg_{tg_id}"
        ok = panel_add_client(new_uuid, email, u["device_limit"], str(tg_id))
        if not ok:
            return False, "Не удалось создать клиента в панели."
        u["uuid"] = new_uuid
    else:
        panel_update_client_limit(u["uuid"], u["device_limit"])
...
def pwa_login_text(code: str, base_url: str) -> str:
    return (
        "✅ <b>Вход в приложение подтвержден</b>\n\n"
        "🔐 <b>Код входа:</b>\n"
        f"<code>{code}</code>\n"
        "⏱ Действует: <b>15 минут</b>\n\n"
        "📱 <b>Как открыть правильно (iPhone):</b>\n"
        "1. Открой ссылку в <b>Safari</b>\n"
        "2. Введи код\n"
        "3. Добавь на экран «Домой»\n\n"
        f"🌐 <b>PWA:</b> {base_url}"
    )
...
async def xray_health_check():
    ...
    while True:
        ...
        if status != 0:
            ...
            await notify_admins(bot, "⚠️ Ядро Xray упало, но я его успешно перезапустил!")
        ...
        if today >= remind_from and days_left >= 0:
            ...
            if last_reminder != today_str:
                await notify_admins(bot, f"📢 Хозяин, пора оплатить сервер (Aeza)! Осталось дней: {days_left}")
            ...
            for uid, rec in list(db.items()):
                ...
                if usage >= limit_bytes and rec.get("status") != "traffic_block":
                    ...
                    try:
                        ...
                        pass
                    await notify_admins(bot, f"⛔ Пользователь {uid} достиг лимита трафика ({limit_gb} GB).")
                db[uid] = rec
                changed = True
            if changed:
                save_db(db)
...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        ...
        "Добро пожаловать в закрытый клуб свободного интернета.\n\n"
        "Мы полностью переехали в наше **Mini App (PWA)**, где ты сможешь:\n"
        "📱 Управлять своими устройствами (добавлять/удалять)\n"
        "🚀 Копировать ключи для подключения\n"
        "💳 Продлевать подписку\n"
        "💬 Общаться с поддержкой\n\n"
        "<b>Как начать:</b>\n"
        "1. Нажми кнопку <b>«🌟 Открыть приложение»</b> ниже\n"
        "2. (Рекомендуется) Нажми кнопку «Поделиться» -> «На экран Домой» (для iOS) или «Добавить на гл. экран» (для Android)\n"
        "3. Внутри приложения перейди в «Устройства» и нажми «Добавить устройство», чтобы получить ключ.\n\n"
        "Твой тестовый период: <b>7 дней</b>."
    )
    
    # PWA login flow: /start pwa
    if args and args.strip().lower() == "pwa":
        existing = db.get(str(user_id))
        st = (existing or {}).get("status", "none")
        if not is_admin(user_id) and st in ("pending", "denied", "none"):
            return await message.answer("⛔ Сначала получи доступ в клуб через приглашение.")
        base = pwa_base_url()
        if not base:
            return await message.answer("⚠️ PWA URL не настроен.")
        login_token = issue_pwa_login_token(str(user_id))
        one_time_code = issue_pwa_code(str(user_id))
        login_url = build_url_with_query(base, login_token=login_token)
        kb_login = InlineKeyboardMarkup().add(InlineKeyboardButton("🔐 Открыть кабинет", url=login_url))
        return await message.answer(
            pwa_login_text(one_time_code, base),
    ...
    if is_admin(user_id):
        return await message.answer(
            welcome_text,
            parse_mode="HTML",
            reply_markup=user_kb("active", is_admin=True, user_id=str(user_id)),
            disable_web_page_preview=True
        )
            ...
            if ref_id and ref_id.isdigit():
                ...
                if str(user_id) not in db:
                    ...
                    return await message.answer(
                        ...
                        reply_markup=user_kb("pending", user_id=str(user_id))
                    )

                existing = db.get(str(user_id), {})
                st = existing.get("status", "pending")
                if st == "pending":
                    return await message.answer("⏳ Заявка уже на рассмотрении.", reply_markup=user_kb("pending", user_id=str(user_id)))
                if st == "denied":
                    return await message.answer("🚫 Доступ отклонен.", reply_markup=user_kb("denied", user_id=str(user_id)))
                if st == "approved":
                    return await message.answer("✅ Вход в клуб открыт. Выбери тариф в Mini App.", reply_markup=user_kb("approved", user_id=str(user_id)))
                if st in ("trial", "active", "vip"):
                    return await message.answer(welcome_text, parse_mode="HTML", reply_markup=user_kb(st, user_id=str(user_id)), disable_web_page_preview=True)
                if st in ("expired", "banned", "traffic_block"):
                    return await message.answer("⛔ Доступ сейчас неактивен. Напиши в поддержку.", reply_markup=user_kb(st, user_id=str(user_id)))
        # если пользователь уже в базе, даем меню по статусу
        existing = db.get(str(user_id))
        if existing:
            st = existing.get("status", "pending")
            if st == "pending":
                return await message.answer("⏳ Заявка на рассмотрении.", reply_markup=user_kb("pending", user_id=str(user_id)))
            if st == "denied":
                return await message.answer("🚫 Доступ отклонен.", reply_markup=user_kb("denied", user_id=str(user_id)))
            if st == "approved":
                return await message.answer("✅ Вход в клуб открыт. Выбери тариф в Mini App.", reply_markup=user_kb("approved", user_id=str(user_id)))
...
@dp.message_handler(commands=['pwa'])
async def cmd_pwa_login(message: types.Message):
    user_id = message.from_user.id
    db = load_db()
    existing = db.get(str(user_id))
    st = (existing or {}).get("status", "none")
...
async def cmd_panel_otp(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
    ...
    await message.answer(
        ...
        "1. Открой **Mini App**\n"
        "2. Перейди во вкладку **Доступ**\n"
        "3. Нажми кнопку **Разблокировать доступ**",
        parse_mode="Markdown"
    )


# Старый метод process_get_key удален - выдача ключей перенесена в PWA
...
@dp.message_handler(lambda message: not is_admin(message.from_user.id) and not message.text.startswith('/'))
async def forward_to_admin(message: types.Message):
    kb = InlineKeyboardMarkup().add(InlineKeyboardButton("Ответить", callback_data=f"reply_{message.from_user.id}"))
    await notify_admins(bot, f"📧 Сообщение от @{message.from_user.username} (ID: {message.from_user.id}):\n\n{message.text}", reply_markup=kb)
...
async def prepare_reply(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, f"Введите ответ для пользователя {user_id} (или /cancel):")
...
async def approve_user(callback_query: types.CallbackQuery):
    ...
    if user_id not in db:
        return await notify_admins(bot, "Пользователь не найден.")
    ok, err = ensure_user_trial(db, user_id, email_hint=f"tg_{user_id}")
    if not ok:
        return await notify_admins(bot, f"⚠️ Не удалось одобрить: {err}")
    save_db(db)
    user_name = db[user_id].get("name") or user_id
    await notify_admins(bot, f"✅ Добавил нового пользователя: {user_name} ({user_id}) · trial {TRIAL_DAYS} дней")
    await bot.send_message(
        int(user_id),
        f"✅ Твоя заявка одобрена. Тебе сразу выдан trial на {TRIAL_DAYS} дней.\nНажми «🚀 Получить ключ» или открой Mini App.",
        reply_markup=user_kb("trial", user_id=str(user_id))
    )
...
async def admin_menu(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "💻 Панель управления GhostLink:", reply_markup=kb)
...
async def admin_cat_users(callback_query: types.CallbackQuery):
    ...
    kb.add(InlineKeyboardButton("⬅️ Назад", callback_data="admin_panel"))
    await notify_admins(bot, "👥 Пользователи:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_cat_comm(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "📣 Коммуникации:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_cat_sec(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "🛡 Безопасность:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_cat_sys(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "⚙️ Система:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_cat_vip(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "👑 VIP:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_cat_backup(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "🗄 Бэкапы:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def admin_broadcast(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Отправь текст рассылки (или /cancel):")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_panel_unlock(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи свой внешний IP (например 1.2.3.4):")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_extend_panel(callback_query: types.CallbackQuery):
    ...
    if not ip:
        await notify_admins(bot, "Нет активного IP. Открой доступ сначала.")
    ...
    if ok:
        ...
        await notify_admins(bot, f"Продлил доступ на 10 минут для {ip}.")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_ban_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи Telegram ID пользователя для блокировки:")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_trial7_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи Telegram ID пользователя для trial 7 дней:")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_extend_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи: <ID> <дни> (пример: 123456789 30)")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_unlimited_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи Telegram ID пользователя для доступа без срока:")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_traffic_limit_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи: <ID> <GB в месяц> (пример: 123456789 200)")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_unban_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи Telegram ID пользователя для разблокировки:")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_delete_user(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Введи Telegram ID пользователя для удаления:")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_delete_vip(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "Подтверди удаление VIP: напиши YES")
    await bot.answer_callback_query(callback_query.id)
...
async def admin_vip_menu(callback_query: types.CallbackQuery):
    ...
    await notify_admins(bot, "👑 Управление VIP:", reply_markup=kb)
    await bot.answer_callback_query(callback_query.id)
...
async def handle_admin_text(message: types.Message):
    ...
    if admin_state["mode"] in ("trial7_user", "unlimited_user"):
        ...
        if admin_state["mode"] == "trial7_user":
            ...
            try:
                await bot.send_message(int(user_id), "🎁 Тебе выдан trial на 7 дней. Теперь кнопка «🚀 Получить ключ» активна.", reply_markup=user_kb("trial", user_id=str(user_id)))
            except Exception:
                pass
        else:
            u["expiry"] = None
            ...
            try:
                await bot.send_message(int(user_id), "✅ Тебе выдан доступ без срока.", reply_markup=user_kb("active", user_id=str(user_id)))
            except Exception:
                pass
        admin_state["mode"] = None
        return
    ...
    if admin_state["mode"] in ("ban_user", "unban_user", "delete_user"):
        ...
        if not _is_int(user_id):
            await message.answer("Нужен числовой Telegram ID.")
            return
        if is_admin(user_id) and admin_state["mode"] in ("ban_user", "delete_user"):
...
async def schedule_panel_autolock(delay_sec=600):
    try:
        ...
        await notify_admins(bot, f"⏱ Панель закроется через 1 минуту. IP: {ip or '-'}", reply_markup=kb)
        await asyncio.sleep(60)
        ok, msg = panel_lock()
        if ok:
            await notify_admins(bot, "⏱ Панель автоматически закрыта.")
